...
public class PoiBigExcelUtil extends DefaultHandler {  
     ...
     */  
    private int k = 0;
    public ArrayList<ArrayList<Object>> list = new ArrayList<ArrayList<Object>>();
    ...
    public void optRow(int sheetIndex, int curRow, List<String> rowList) {  
        ...
        for(int i=0;i<rowList.size();i++) {
        	if(i == 15 || i == 16){
        		try{
        			tmpdoub = Double.parseDouble(rowList.get(i));
        			temp = df.format(tmpdoub);
        		}catch(Exception e){
        			temp = "0";
        		}
        	}else{
    ...
    public static void main(String[] args) {
    	PoiBigExcelUtil pe = new PoiBigExcelUtil();
    	try {
			pe.readOneSheet("F:\\ec.xlsx");
		} catch (Exception e) {
			e.printStackTrace();
		}
    	System.out.println(pe.list.get(1));
    	System.out.println(pe.list.get(2));
